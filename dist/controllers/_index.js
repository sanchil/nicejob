...
const readOneCtrl = (req, res, next) => {
  if (req.params.id) {
    queries.readOne(req.params.collection, req.params.id).then(doc => {
      if (doc.exists) {
        res.json(doc.data());
      } else {
    ...
    res.json({
      "error": "Get Doc failed. Improper Id"
    });
  }
};
...
const readManyCtrl = (req, res, next) => {
  if (req.params.collection) {
    ...
    queries.readMany(req.params.collection, lim).then(snapshots => {
    ...
    res.json({
      "error": "Get Doc collections failed. Improper Id"
    });
  }
};
